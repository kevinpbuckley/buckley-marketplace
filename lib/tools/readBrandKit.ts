...
const inputSchema = z.object({
  ...
  section: z
    ...
    .optional()
    .describe(
      'Section name or id, e.g. "Tone of Voice". Omit to get the kit summary and its section names.'
    ),
});
...
export const execute: ToolExecutor = async (input, context) => {
  ...
  try {
    ...
    if (!brandKitId) {
      const kits = (await hostFetch(client, `${AGENT_API}/api/v1/brandkits`)) as BrandKit[];
      return {
        success: true,
        output: {
    ...
    const kit = (await hostFetch(client, `${AGENT_API}/api/v1/brandkits/${brandKitId}`)) as BrandKit;
    ...
    const match =
      sections.find((s) => s.id.toLowerCase() === needle) ??
